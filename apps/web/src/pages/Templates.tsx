...
export function Templates() {
  ...
  const [formData, setFormData] = useState({ name: '', subject_template: '', html_template: '' });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      setIsAddOpen(false);
      setEditId(null);
      setFormData({ name: '', subject_template: '', html_template: '' });
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
        {templates.map(t => (
          <div key={t.id} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
            <h3 style={{ fontSize: '1.125rem', fontWeight: 600, marginBottom: '0.5rem' }}>{t.name}</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '1rem', fontWeight: 500 }}>
              {t('Subject')}: {t.subject_template}
            </p>
            <div style={{ backgroundColor: 'var(--bg-dark)', padding: '1rem', borderRadius: '0.5rem', fontSize: '0.875rem', color: 'var(--text-muted)', whiteSpace: 'pre-wrap', flex: 1, marginBottom: '1rem' }}>
              {t.html_template}
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="btn btn-outline" onClick={() => openEdit(t)} style={{ flex: 1 }}>{t('Edit Template')}</button>
              <button className="btn-outline" onClick={() => handleDelete(t.id)} style={{ padding: '0.5rem', color: 'var(--danger)', background: 'transparent', border: '1px solid var(--border-color)', borderRadius: '0.5rem', cursor: 'pointer' }} title="Delete">
                <Trash2 size={18} />
              </button>
            </div>
          </div>
